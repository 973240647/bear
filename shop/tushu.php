...
header("Content-Type:text/html;charset=utf-8");
	$page_size=6;
	$sql1="select count(*) as amount from product,main_type where main_type.t_id=product.p_type and main_type.t_type like '%图书%' ";
...
<?php
			if($amount>0)
			{
				...
				$sql="select distinct * from product,main_type where main_type.t_id=product.p_type and main_type.t_type like '%图书%' order by p_time desc limit $start,$page_size";
				$rs=mysql_query($sql);
